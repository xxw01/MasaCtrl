...
# MODIFIED: Add new class for enhanced editability
class MutualSelfAttentionControlEnhanced(AttentionBase):
    MODEL_TYPE = {
        "SD": 16,
        "SDXL": 70
    }
    ...
    def __init__(self, start_step=4, start_layer=10, layer_idx=None, step_idx=None, total_steps=50, model_type="SD"):
        """
        Enhanced Mutual self-attention control with 3 paths: Ls, Lt, Lm
        Args:
            start_step: the step to start mutual self-attention control
            start_layer: the layer to start mutual self-attention control
            layer_idx: list of the layers to apply mutual self-attention control
        ...
        self.layer_idx = layer_idx if layer_idx is not None else list(range(start_layer, self.total_layers))
        self.step_idx = step_idx if step_idx is not None else list(range(start_step, total_steps))
        print("Enhanced MasaCtrl at denoising steps: ", self.step_idx)
        print("Enhanced MasaCtrl at U-Net layers: ", self.layer_idx)
    ...
    def forward(self, q, k, v, sim, attn, is_cross, place_in_unet, num_heads, **kwargs):
        """
        Enhanced attention forward function with 3 paths
        """
        if is_cross or self.cur_step not in self.step_idx or self.cur_att_layer // 2 not in self.layer_idx:
            return super().forward(q, k, v, sim, attn, is_cross, place_in_unet, num_heads, **kwargs)
        ...
        # Further split conditional parts into 3 paths: Ls, Lt, Lm
        qu_s, qu_t, qu_m = qu.chunk(3)
        qu_c_s, qu_c_t, qu_c_m = qc.chunk(3) 
        ku_s, ku_t, ku_m = ku.chunk(3)
        ku_c_s, ku_c_t, ku_c_m = kc.chunk(3)
        vu_s, vu_t, vu_m = vu.chunk(3)
        vu_c_s, vu_c_t, vu_c_m = vc.chunk(3)

        # Path Ls (source/reconstruction): normal self-attention  
        out_u_s = self.attn_batch(qu_s, ku_s, vu_s, sim[:num_heads], attnu, is_cross, place_in_unet, num_heads, **kwargs)
        out_c_s = self.attn_batch(qu_c_s, ku_c_s, vu_c_s, sim[:num_heads], attnc, is_cross, place_in_unet, num_heads, **kwargs)

        # Path Lt (target/editing): Qt attends to Ks, Vs (mutual self-attention)
        out_u_t = self.attn_batch(qu_t, ku_s, vu_s, sim[:num_heads], attnu, is_cross, place_in_unet, num_heads, **kwargs)
        out_c_t = self.attn_batch(qu_c_t, ku_c_s, vu_c_s, sim[:num_heads], attnc, is_cross, place_in_unet, num_heads, **kwargs)

        # MODIFIED: Path Lm (mixing): Use Qs, Ks from source, Vm from mixing path
        out_u_m = self.attn_batch(qu_s, ku_s, vu_m, sim[:num_heads], attnu, is_cross, place_in_unet, num_heads, **kwargs)
        out_c_m = self.attn_batch(qu_c_s, ku_c_s, vu_c_m, sim[:num_heads], attnc, is_cross, place_in_unet, num_heads, **kwargs)

        # Concatenate all paths
        out = torch.cat([out_u_s, out_u_t, out_u_m, out_c_s, out_c_t, out_c_m], dim=0)
